...
Пользователь ↔ Бот MAX ↔ API Apex (авторизация / проверка пользователей)
                        ↔ SMTP (отправка кодов / заявок на почту СД / согласования)
                        ↔ Assyst (входящие webhooks: статусы, закрытие, согласования)
...
| 3 | Создание заявки | Пользователь | MAX → Бот → SMTP (почта СД) → СД → Assyst |
...
**При подтверждении**: бот формирует письмо с данными заявки (описание, фото) и отправляет его на **почту службы поддержки (СД)** через SMTP. СД регистрирует обращение в Assyst вручную. Предварительная запись заявки сохраняется в БД бота со статусом «Отправлена» и `source = 'bot'`. После того как СД зарегистрирует заявку в Assyst, бот получает webhook `type: new` и обновляет запись полями `incident_id` и `event_reference`.
...
database "БД бота" as DB
participant "SMTP" as S
actor "Служба поддержки (СД)" as SD
...
alt Валидация пройдена
    ...
    alt Пользователь подтверждает
        ...
        B -> DB : Сохранение заявки (source=bot, статус=Отправлена)
        DB --> B : OK
        B -> S : Отправка письма на почту СД\n(описание + фото)
        S --> B : OK
        B --> U : Заявка успешно создана!

        note over SD, AS : СД регистрирует заявку в Assyst
        SD -> AS : Создание заявки
        AS -> B : Webhook (type: new)\nincidentId, eventReference
        B -> DB : Обновить запись\n(incident_id, event_reference)
...
1. Проверить, есть ли в БД предварительная запись с `source = 'bot'` для данного пользователя (по `email`). Если да — обновить существующую запись полями `incidentId`, `eventReference`, `status`. Если нет — создать новую запись в БД (`source = 'assyst'`), т.к. заявка была создана в Assyst напрямую (не через бот).
